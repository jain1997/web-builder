...
class AgentState(TypedDict, total=False):
    ...
    file_plan: list[dict]
    # Image plan from planner:
    # [{"path": "images/hero.png", "prompt": "modern hero banner ..."}]
    ...
    # Parallel image generation — each image_generator appends one {path: data_uri}.
    generated_image_parts: Annotated[list[dict], operator.add]

    # Per-generator context injected by Send — which file/image to generate.
    current_file: dict   # {"path": str, "description": str}
    current_image: dict  # {"path": str, "prompt": str}
